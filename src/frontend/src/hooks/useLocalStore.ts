...
const OWNER_PROFILE_KEY = "jomibazar_owner_profile";

const DEFAULT_OWNER_PROFILE: OwnerProfile = {
  name: "",
  title: "সাইট অ্যাডমিন ও মালিক",
  phone: "",
  whatsapp: "",
  email: "",
  address: "",
...
export function useOwnerProfile() {
  ...
  const refresh = useCallback(() => setProfile(getOwnerProfile()), []);
  ...
  const update = useCallback((updates: Partial<OwnerProfile>) => {
    const current = getOwnerProfile();
    const merged = { ...current, ...updates };
    saveOwnerProfile(merged);
    setProfile(merged);
  }, []);
